...
for j in range(2):
    pt = perts[j]
    f = "{}_pf_{}_{}_cycle{}_oberr{}.npy".format(model, op, pt, 0, oberr)
    if not os.path.isfile(f):
        print("not exist {}".format(f))
        exit
    pf = np.load(f)
    ymin = np.min(pf)*1.001
    ymax = np.max(pf)*1.001
    ylim = max(np.abs(ymin),ymax)
    mappable0 = ax[j].pcolor(x,x,pf,cmap=cmap,norm=Normalize(vmin=-ylim,vmax=ylim))
...
fig.tight_layout()
fig.savefig("{}_pf_{}_oberr{}.png".format(model,op,oberr))