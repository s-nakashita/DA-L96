...
C
     ...
     *                  XOUT,GOUT,DOUT,GOLDOUT,STPK,OFLAG,OFINISH)
C
C Subroutine parameters
C
      DOUBLE PRECISION, INTENT(INOUT) :: X(N),G(N),D(N),GOLD(N),W(N)
      DOUBLE PRECISION, INTENT(OUT) :: XOUT(N),GOUT(N),DOUT(N),
     *                                 GOLDOUT(N)
      DOUBLE PRECISION, INTENT(OUT) :: STPK
      DOUBLE PRECISION, INTENT(IN) :: F,EPS
      INTEGER, INTENT(IN) ::  N,IPRINT(2),IREST,METHOD
...
C
     ...
     .       DGOUT,DG,DG1
      INTEGER MP,LP,ITER,NFUN,MAXFEV,INFO,I,NFEV,NRST,IDES,IM,NDES
      LOGICAL NEW
C For line search output variables
      INTEGER INFOT 
      DOUBLE PRECISION XTRI(N)
      DOUBLE PRECISION STPTRI
C
