...
#    for method in methods:
        ...
        for lag in lags:
            ...
            for count in range(1, 51):
                ...
                #f = "linear_oberr/{}_{}_oberr{}_{}_{}.txt".format(op, pt, oberr, method, count)
                f = "t0c_oberr/{}_{}_oberr00100_t0c{}_{}.txt".format(op, pt, lag, count)
...
#            enrs[i] = np.mean(nrstdv[5:])
            ...
            fm = "t0c_oberr/{}_{}_oberr00100_t0c{}_mean.txt".format(op, pt, lag)
            ...
            fs = "t0c_oberr/{}_{}_oberr00100_t0c{}_stdv.txt".format(op, pt, lag)
...
#        print(enrs)
        ...
        if label[-2:] == "jh":
            ...
            style=linestyle0[0]
        #ax.plot(obs_s, er, linestyle=style, color=linecolor0[pt], label=label)
        ax.plot(lags, er, linestyle=style, color=linecolor0[pt], label=label)
...
#        title=op)#+" "+method)
    ...
    ax.plot(lags, y, linestyle="dotted", color="tab:gray")
    ax.set(xlabel="initial t0c", ylabel="RMSE",
        title=op)
#ax.set_ylim(0.0, 0.1)
#    ax.set_xscale("log")
    ax.set_yscale("log")
...
#ax.set_xticks(x, minor=True)
    ax.legend()#ncol=3)
    fig.savefig("{}_error_{}_t0c.png".format(model, op))#, method))
    fig.savefig("{}_error_{}_t0c.pdf".format(model, op))#, method))
